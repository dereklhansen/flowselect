...
analyze_linear_model <- function(cfg) {
  ...
  for (dataset in c(
    "gaussian_ar_mixture_.*_d100_n100000_rho0.98_",
    ...
    "rnasq_cor_.*_a40_n10_nv100_nvc5_nt100000_"
    ...
    ggsave(
      outdir %s+% "/" %s+% dataset %s+% "_fdr_and_power_hrt.png",
      fdr_plot,
      width = 8,
    ...
    fdr_boxplot <- plot_tbl %>%
      ...
      mutate(Knockoff = factor(Knockoff, c("DeepKnockoff", "KnockoffGAN", "MASS", "MDN", "DDLK", "FlowSelect"))) %>%
    ...
    ggsave(
      outdir %s+% "/" %s+% dataset_out %s+% "_boxplots_hrt.png",
      fdr_boxplot,
      width = 8,
      height = 6,
      scale=0.65,
    ...
    dataset_out <- stri_replace_all_fixed(dataset, ".*_", "")
    ggsave(
      outdir %s+% "/" %s+% dataset_out %s+% "_meanplots_hrt.png",
      fdr_meanplot,
      width = 4,
      height = 6,
      scale=0.65,
